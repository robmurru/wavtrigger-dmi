...
float velocityCurve = 0.5;  // Log
//float velocityCurve = 1.0;  // Linear
//float velocityCurve = 2.0;  // Exp
...
void NoteOn(byte channel, byte pitch, byte velocity) {
  ...
  for (int i = 0; i < 16; i++) {
    if (pitch == midi_notes[i]) {
      float vNorm = (float)velocity / 127.0;              // Normalization
      float curved = pow(vNorm, velocityCurve);           // Curve application
      int gain = (int)(-70 + (curved * (0 - (-70))));    // Remapping
